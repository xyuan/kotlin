...
class CallCompleter(
    ...
    private val supertypesChecker: SupertypesChecker
) {
    fun <D : CallableDescriptor> completeCall(
        ...
        if (context.trace.wantsDiagnostics()) {
            if (resolvedCall == null) {
                checkMissingSupertypes(context, supertypesChecker)
            } else {
                ...
                val callCheckerContext = CallCheckerContext(context, deprecationResolver, moduleDescriptor, supertypesChecker)
    ...
    private fun checkMissingSupertypes(context: BasicCallResolutionContext, supertypesChecker: SupertypesChecker) {
        ...
        MissingDependencySupertypeChecker.checkSupertypes(
            explicitReceiver.type, call.callElement, context.trace, supertypesChecker
        )
    }
